...
> 前置：[01-core-specification.md](01-core-specification.md)（核心原则）、[04-security-governance.md](04-security-governance.md)（安全与治
理）、[06-cicd-pipeline.md](06-cicd-pipeline.md)（CI/CD Pipeline）

---

## 第 0 章：为什么需要本规范

大规模 Auto-Coding = 每天数十至数百个 AI 生成的 PR 进入 Pipeline。传统安全测试面临：

- **速度不匹配**：人工渗透测试周期为周/月级，无法跟上 AI 日级发布节奏
- **覆盖盲区**：AI 生成代码引入新型攻击面（Prompt 注入、训练数据泄漏、模型输出操控）
- **韧性缺失**：微服务 + AI 服务依赖链变长，单点故障传播路径不可预测
- **合规滞后**：GDPR/SOC2 审计为季度/年度级，Auto-Coding 需要持续合规
    ...
    steps:
      ...
      - name: OWASP ZAP Baseline Scan
        ...
        with:
          ...
          fail_action: true  # Critical/High 阻断
          cmd_options: '-a -j'  # 全面扫描 + AJAX 支持
...
| 报告分析 | 汇总扫描结果，生成风险评级 | 人工确认 Critical/High 评级 |
| 修复建议 | 生成修复代码 + 验证测试 | 修复必须经人工审查（P4） |

#### 2.4.2 AI 辅助扫描工作流

```
1. AI 分析代码变更 → 生成漏洞假设列表
2. AI 生成针对性 Nuclei/ZAP 测试配置
3. 自动化扫描执行（隔离环境）
4. 结果汇总 + 误报过滤
5. AI 生成修复方案（含测试用例）
6. 人工审查修复方案 → 合并
...
## 修复计划
- Critical: {deadline}（24 小时内）
- High: {deadline}（72 小时内）
- Medium: {deadline}（下次发布前）
```
...
| 故障类型 | 工具 | 注入方式 | 验证目标 |
|---------|------|---------|---------|
| 节点宕机 | Chaos Mesh / Litmus | Kill Pod / EC2 终止 | 服务迁移、负载均衡 |
| 网络延迟 | tc / Toxiproxy | 注入延迟 100ms-5s | 超时配置、用户体验 |
| 网络分区 | Chaos Mesh | iptables 规则 | 分区容忍、数据一致性 |
| 磁盘满 | Chaos Mesh | 填充磁盘至 95% | 日志轮转、告警触发 |
| CPU 耗尽 | stress-ng | CPU 100% 占用 | 限流、降级策略 |
| 内存泄漏 | Chaos Mesh | 内存压力注入 | OOM Killer、重启恢复 |

#### 3.3.2 应用层故障

| 故障类型 | 工具 | 注入方式 | 验证目标 |
|---------|------|---------|---------|
| 依赖服务失效 | Gremlin / Pumba | 模拟下游超时/500 | 熔断器、降级响应 |
| 数据库连接中断 | Toxiproxy | 切断 DB 连接 | 连接池、重试逻辑 |
| 缓存失效 | 自定义脚本 | Flush Redis | 缓存穿透、DB 保护 |
| 消息队列积压 | RabbitMQ CLI | 暂停消费者 | 背压处理、消息丢失 |
| 配置错误 | 自定义脚本 | 注入错误配置值 | 配置校验、默认值 |
| AI 模型超时 | 自定义代理 | 模拟模型 API 超时 | AI 降级策略 |
...
|------|------|------|---------|
| D0 | 正常 | 完整功能 | 无 |
| D1 | 性能降级 | 降低非关键功能优先级（如推荐、分析） | 轻微（部分功能延迟） |
| D2 | 功能降级 | 关闭非核心功能，保留核心路径 | 明显（部分功能不可用） |
| D3 | 只读模式 | 禁止所有写操作，返回缓存数据 | 严重（无法创建/修改） |
| D4 | 完全不可用 | 返回友好的错误页面 | 完全中断 |

#### 3.5.2 AI 生成代码的降级要求

AI 生成的每个服务 **必须** 实现：

1. **健康检查端点**（`/health`）：返回服务自身 + 关键依赖的状态
2. **降级开关**：支持运行时切换降级等级
3. **超时配置**：所有外部调用必须有超时（默认值见第 3.6 节）
4. **重试策略**：指数退避 + 最大重试次数 + 抖动
5. **熔断器**：连续失败 N 次后自动熔断，定时试探恢复

### 3.6 超时与重试规范

#### 3.6.1 超时默认值

| 调用类型 | 连接超时 | 读取超时 | 总超时 | 依据 |
|---------|:-------:|:-------:|:------:|------|
| 内部服务（同集群） | 100ms | 500ms | 1s | 网络延迟 < 10ms |
| 内部服务（跨集群） | 500ms | 2s | 5s | 网络延迟 < 200ms |
| 数据库 | 200ms | 1s | 3s | 查询 SLA |
| 缓存 | 50ms | 200ms | 500ms | 内存操作 |
| 外部 API | 1s | 5s | 10s | 第三方不可控 |
| AI 模型 API | 2s | 30s | 60s | 模型推理时间 |
| 文件存储 | 500ms | 5s | 10s | I/O 操作 |

**规则**：超时值不得由 AI 自行决定，必须从配置中心读取。硬编码超时值 = 违反 P12。
...
- 固定间隔重试（无指数退避）
- 无抖动重试（导致同步重试风暴）
- 对 4xx 错误重试（客户端错误不应重试）
- 对 500 错误重试（内部服务器错误应走熔断器）
...
| **间接 Prompt 注入** | AI-S02 | 通过第三方数据（网页/文档）注入恶意指令 | 沙箱化执行 + 输出过滤 | High |
| **训练数据泄漏** | AI-S03 | 模型输出包含训练数据中的敏感信息 | 正则匹配 + 指纹扫描 | Critical |
| **系统提示词泄漏** | AI-S04 | 用户诱导模型暴露系统 Prompt | 对抗测试 | High |
| **工具调用滥用** | AI-S05 | AI 生成的代码过度使用或误用工具 | 工具调用审计 + 权限验证 | High |
| **输出操控** | AI-S06 | 恶意用户操控模型输出影响下游系统 | 输出签名 + 校验和 | Medium |
| **上下文窗口溢出** | AI-S07 | 超长输入导致上下文溢出或截断 | 输入长度限制 + 分块处理 | Medium |
| **模型版本漂移** | AI-S08 | 模型升级导致行为不一致 | 版本锁定 + 回归测试 | Medium |
| **AI 生成恶意代码** | AI-S09 | AI 无意中生成包含漏洞的代码 | SAST + 人工审查（P4） | High |
| **权限提升** | AI-S10 | AI 生成的认证/授权逻辑存在绕过 | 权限测试 + 边界验证 | Critical |
...
间接注入发生在 AI 处理外部数据（网页内容、用户上传文档、API 响应）时，数据中隐藏的指令被模型执行。
...
```
┌─────────────────────────────────────────────┐
│           合规策略引擎（Policy Engine）        │
├─────────────────────────────────────────────┤
│  GDPR  │  SOC2  │  HIPAA  │  PCI-DSS  │ 自定义 │
├─────────────────────────────────────────────┤
│           规则编译器（Rules Compiler）         │
├─────────────────────────────────────────────┤
│  代码扫描  │  配置检查  │  数据审计  │  日志验证  │
├─────────────────────────────────────────────┤
│           合规报告 + 证据链（.gate/）          │
...
**核心原则**：合规不是一次性审计，而是持续自动化验证。每个 PR 都必须通过合规检查。
...
| # | 条款 | 检查项 | 验证方式 | 工具 |
|---|------|-------|---------|------|
| G01 | 第 5 条 | 数据最小化：不得收集超出必要范围的个人数据 | 代码扫描 + 数据流分析 | Semgrep + 自定义规则 |
| G02 | 第 6 条 | 合法基础：每个数据处理操作必须有合法基础记录 | 配置检查 | 合规策略引擎 |
| G03 | 第 7 条 | 同意管理：用户同意必须可记录、可撤回 | 端到端测试 | Playwright / Cypress |
| G04 | 第 15 条 | 访问权：用户可请求导出其个人数据 | API 测试 | 自动化 API 测试套件 |
| G05 | 第 17 条 | 被遗忘权：用户数据可完全删除 | 数据库验证 | 数据审计脚本 |
| G06 | 第 25 条 | 隐私默认：默认配置必须是最隐私友好的 | 配置检查 | OPA / Checkov |
| G07 | 第 30 条 | 处理记录：必须维护数据处理活动记录 | 日志验证 | 日志审计脚本 |
| G08 | 第 32 条 | 安全处理：个人数据必须加密存储和传输 | SAST + 配置检查 | gitleaks + SSLyze |
| G09 | 第 33 条 | 数据泄露通知：72 小时内通知监管 | 流程验证 | 应急演练脚本 |
| G10 | 第 44 条 | 跨境传输：数据不得传输到无充分保护的国家 | 网络流量分析 | 代理日志分析 |
  ...
  gdpr-check:
    ...
    steps:
      - name: Data Flow Analysis
        run: |
          # 扫描个人数据处理路径
          semgrep --config .security/gdpr-rules.yml \
            --json -o .gate/gdpr-dataflow.json

      - name: Encryption Check
        run: |
          # 验证个人数据加密
          checkov -d . --check CKV_SECRET_* \
            --framework all \
            --output json --output-file-path .gate/gdpr-encryption.json

      - name: Consent Flow Test
        run: |
          # 端到端验证同意管理流程
          npx playwright test tests/compliance/consent-flow.spec.ts

      - name: Right to Erasure Test
        run: |
          # 验证数据删除完整性
          python .security/scripts/gdpr-erasure-test.py \
            --user test-user-001 \
            --output .gate/gdpr-erasure.json

      - name: Generate Compliance Report
        run: |
          python .security/scripts/compliance-reporter.py \
            --inputs .gate/gdpr-*.json \
            --output .gate/gdpr-compliance-report.md
```

### 5.3 SOC2 合规检查

#### 5.3.1 SOC2 Trust Service Criteria 映射

| 类别 | 标准 | 自动化验证 | 频率 |
|------|------|-----------|------|
| **安全性** | CC6.1 逻辑访问控制 | 权限配置扫描 + 越权测试 | 每次 PR |
| **安全性** | CC6.6 外部威胁防护 | DAST + WAF 日志分析 | 每日 |
...
检测到违规
    │
    ├── Critical → 立即阻断发布 + 通知安全团队 + 24 小时内修复
    ├── High     → 阻断发布 + 72 小时内修复
    ├── Medium   → 警告 + 下次发布前修复
    └── Low      → 记录 + 计划修复
```
...
| 级别 | 条件 | 响应时间 | 处理流程 |
|------|------|---------|---------|
| P0-Critical | 数据泄漏、远程代码执行、认证完全绕过 | 15 分钟内响应 | 立即下线 → 修复 → 验证 → 重新上线 |
| P1-High | SQL 注入、XSS、权限提升 | 1 小时内响应 | WAF 规则 → 修复 → 验证 |
| P2-Medium | 信息泄漏、CSRF、配置错误 | 24 小时内修复 | 排入修复队列 → 修复 → 验证 |
| P3-Low | 安全头缺失、过时依赖 | 下次发布前修复 | 排入 backlog |
