...
```
main merge → staging deploy → smoke test → canary (5%) → canary (25%) → canary (50%) → production (100%)
```

| 阶段 | 触发条件 | 验证项 | 自动/人工 |
|------|---------|--------|----------|
| **staging deploy** | PR merged to main | 构建镜像、部署到 staging | 自动 |
| **smoke test** | staging 部署完成 | 健康检查 + 核心 E2E | 自动 |
| **canary 5%** | smoke test 通过 | 错误率 < 1%、P99 延迟 < 基线×1.5 | 自动 |
| **canary 25%** | canary 5% 稳定 10min | 同上 + 业务指标无异常 | 自动 |
| **canary 50%** | canary 25% 稳定 15min | 同上 + 无告警触发 | 自动 |
| **production 100%** | canary 50% 稳定 30min | 最终检查点 | L2/L3 自动，L4 自动 |

**自动回滚触发条件**（任一满足即触发）：
- 错误率 > 5%（5 分钟窗口）
- P99 延迟 > 基线×2（5 分钟窗口）
- 关键业务指标下降 > 10%（如订单量、注册数）
- 健康检查连续 3 次失败
- 告警：Critical 级别触发

**数据一致性保障**：回滚时必须保证数据库向前兼容——新版本的 migration 不得破坏旧版本代码的数据读取。具体规则见数据库迁移规范。
