...
| 禁止文件检查 | 禁止提交 `.env*`、`secrets/`、`credentials/`、`keys/` 及其他 05-tool-reference.md 第 5.3 节列出的 Protected Paths | **是** | 检查 AI 是否生成了敏感文件 |
| pre-send 数据分级扫描 | 检查待提交内容是否含 Restricted 数据 | **是** | P10 数据分级 |

**AI 特有规则**：AI 生成的每次提交必须在 commit message 中包含以下 trailer 格式的元数据：
```
<简短描述>
...
| AC 覆盖率 | 映射 Spec AC 到测试函数 + 证据 | **是** | 100% 覆盖，否则=未完成 |
| 边界覆盖 | 检查正常路径 + 异常路径测试 | **是** | 每个 AC ≥2 种异常路径 |
| 包覆盖率 | `go test -cover` / `nyc` / `coverage.py` | 警告 | 基线 = 上一次 main 分支通过 L4 验证的构建。不得较基线下降 > 5%。基线值存储在 `.gate/performance/baseline-coverage.json` |
| Flaky Test 检测 | 同一 PR 重跑 3 次检测不稳定测试 | 警告 | 02-auto-coding-practices.md 4.3 |

**AI 特有规则**：
- AI 生成测试时，CI 必须检查断言是否"过于宽松"（如 `assert(true)`）
- AC 覆盖率 100% 才是完成的真实指标，包覆盖率仅用于趋势监控
...
**AI Reviewer 自动审查规则**：
1. AI Reviewer 必须使用与代码生成**不同的 API 会话**（session）。建议使用不同模型（如代码生成使用 Sonnet，审查使用 Opus）。禁止同一 session 审查自己生成的代码。
...
| 数据库迁移测试 | 在干净 DB 上执行迁移 + 回滚 + destructive change 检测 | **是** | 见 [08-database-migration.md](08-database-migration.md) |
| **性能基线检查** | 基准测试 vs 上次基线 | **是** | 回归 >5% = 警告，>10% = 阻断（详见 11-performance-baseline.md 第 3 章） |
...
| **安全漏洞不可自修** | SAST 检测的安全漏洞，AI 可以生成修复建议（写入 `.gate/security-fix-suggestion.md`）但不得自动提交修复。修复必须由人工审核确认后方可合并 |
