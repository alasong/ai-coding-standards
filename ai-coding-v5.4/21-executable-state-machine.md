...
PHASE_3_DISPATCH    开发分派 — 按 WBS 拆 Feature → Task 队列
 ...
 ▼
[PHASE_3 子状态循环 — 见 §2.2]
 │  SCFS_BOOT → SCFS_CONTRACT → SCFS_UPSTREAM → TDD_RED → ... → SCFS_WAITING → TASK_GATE
 ▼ (所有 task 完成)
...
### 2.2 PHASE_3 子状态（SCFS 生命周期 + TDD 循环 + Spec-to-Test 验证）

PHASE_3 是状态机中最关键的部分——这里是 AI 写代码的地方。拆成子状态确保 TDD 流程不被跳过，并加入 **SCFS 结构化约束文件生命周期**和 **Spec-to-Test 对齐验证**。

#### 2.2.1 SCFS 任务生命周期（外层包装）
...
| 子状态 | 能写什么 | 不能写什么 | 退出条件 |
|--------|---------|-----------|---------|
| SCFS_BOOT | .omc/state/{task-id}.json | 代码、测试、业务文档 | State Manifest 已创建，phase = "reading_contract" |
| SCFS_CONTRACT | .omc/state/{task-id}.json | 代码、测试 | State Manifest 记录 contract 读取完成 |
| SCFS_UPSTREAM | .omc/state/{task-id}.json | 代码、测试 | State Manifest 记录 upstream context 加载完成 |
| [进入 TDD 循环] | — | — | — |
| SCFS_GATE_REQUEST | .omc/state/{task-id}.json | 任何 | State Manifest gate_status = "requesting_gate" |
| SCFS_WAITING | 无 | 任何 | Gate Report 存在，状态为 pass 或 fail |

**规则**：SCFS_BOOT / SCFS_CONTRACT / SCFS_UPSTREAM 是每个 task 的强制前置序列，不得跳过。

#### 2.2.2 TDD 循环（内层执行）

在 SCFS_UPSTREAM 完成后进入，在 SCFS_GATE_REQUEST 前结束：

```
TDD_RED → TDD_GREEN → TDD_REFACTOR → SPEC_ALIGN_CHECK → SCFS_GATE_REQUEST
 │                                        │
 │ FAIL (≤3) ─────────────────────────────┘ (Self-Correction 回到 TDD_GREEN)
 │ FAIL (>3) → BLOCKED → 等待人工
```
...
| `scfs_state_file_exists` | `.omc/state/{task-id}.json` 存在且包含 `current_phase` | 同 |
| `scfs_contract_read` | State Manifest 中 `phase_transitions` 包含 `reading_contract` | + Task Contract YAML 校验 |
| `scfs_upstream_loaded` | State Manifest 中 `phase_transitions` 包含 `reading_upstream` | + context_files 存在性校验 |
| `scfs_gate_requested` | State Manifest 中 `gate_status = "requesting_gate"` | 同 |
| `scfs_gate_report_exists` | `.gate/gate-{task-id}.json` 存在 | + Gate Checker 独立验证 |
| `scfs_boundary_check` | 对比 State Manifest `files_modified` vs Task Contract `boundary_constraints` | + SCFS YAML Schema 全量校验 |
...
| P8 最小批量 | 01-core-specification.md §1.1 | SCFS boundary_constraints: max_files ≤ 5 |
| Self-Correction ≤ 3 轮 | 01-core-specification.md §2.2 | TASK_GATE 中计数器 |
| SCFS 结构化约束文件 | 04B-structured-constraints.md | SCFS_BOOT → SCFS_CONTRACT → SCFS_UPSTREAM → SCFS_GATE_REQUEST → SCFS_WAITING |
| SCFS 边界验证 | 04B-structured-constraints.md §5 | TASK_GATE exit_condition: scfs_boundary_check |
