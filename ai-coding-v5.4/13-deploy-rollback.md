...
**AI 特有规则**：AI 不得自行选择部署策略。部署策略由 Spec 中的 `deployment_strategy` 字段指定，AI 仅执行。
...
| **Flag 生命周期** | 从代码合并开始计时，≤ 4 周内必须移除 Flag（全量开启或回滚） |
| **Flag 清理** | Flag 全量开启 48 小时后，AI 必须创建 PR 移除 Flag 代码和条件分支 |
