...
> 版本：v1.1 | 2026-04-17
> 定位：v5.2 规范第 4 章（幻觉检测）的全面升级方案
> 方法论：证据链（Chain of Evidence）+ 多维度交叉验证 + 反事实推理
> 变更：基于 v1.0 新增方案类幻觉（T01-T03）和认知类扩展（C07-C08），支持 P23 方案设计阶段幻觉检测
...
| C07 | **方案确定幻觉** | 对方案设计的最优性表现出虚假确定 | "这是唯一正确的方案" 实际存在多种可行方案 | MEDIUM |
| C08 | **知识完备幻觉** | AI 声称已加载完整领域知识，实际遗漏关键知识 | "已理解项目架构" 但未读取 domain-knowledge/ 文件 | HIGH |

### 1.6.1 方案类幻觉（P23 方案设计阶段新增）

| # | 类型 | 说明 | 示例 | 危险级 |
|---|------|------|------|--------|
| T01 | **架构不一致** | 方案设计与现有架构冲突但未标注 | "新增 REST 接口" 但项目统一使用 gRPC | CRITICAL |
| T02 | **需求遗漏** | 方案设计遗漏已确认的验收标准 | 需求有 5 个 AC，方案只覆盖 3 个 | CRITICAL |
| T03 | **过度设计** | 方案设计超出需求范围，引入不必要的复杂性 | 简单 CRUD 引入微服务 + 消息队列 | HIGH |
