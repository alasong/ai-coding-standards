...
流程裁剪档位（S/M/L/XL）解决的是"变更规模"问题。Scale 适配解决的是"代码库规模"问题。两者正交。

### 4.1 四档 Scale

| Scale | 代码量 | 典型场景 | v6.0 适用度 |
|-------|--------|---------|------------|
| **S1** | < 10 万行 | 新项目、微服务、工具库 | 完全适用，完整流程 |
| **S2** | 10-100 万行 | 成熟单体应用 | 适用，需裁剪 |
| **S3** | 100-1000 万行 | 多模块平台、中型 monorepo | 部分适用，执行机制变形 |
| **S4** | > 1000 万行 | 大型 monorepo、多团队协作平台 | 原则保留，执行机制替换 |

### 4.2 核心原则在不同 Scale 下的执行机制

| v6.0 机制 | S1（<10 万行） | S2（10-100 万行） | S3（100-1000 万行） | S4（>1000 万行） |
|-----------|---------------|------------------|-------------------|----------------|
| **P7 Spec 驱动** | markdown 文件 `specs/F{NNN}.md` | markdown 文件 + 模块分组 | 结构化契约 YAML（Contract 文件） | API 契约 + 依赖图节点 |
| **P3 TDD** | 全量测试 | 全量测试 + 模块级覆盖 | 增量测试（依赖图驱动） | 冒烟 + 契约测试 |
| **P2 DCP 门禁** | Gate Checker 读全部代码 | Gate Checker 读模块代码 | 自动化契约验证 | CI 自动门禁 |
| **Agent 会诊** | 三角色并行读全部代码 | 单 Agent 读模块 + Gate 独立 | Domain Agent 分层（见 04-multi-agent §8.6） | 模块自治 + 顶层约束 |
| **Phase 传导** | 手动 impact-log | 手动 impact-log | 依赖图自动传播 | CI 依赖图 + 阻断 |
| **上下文加载** | 全部相关文件 | 按模块加载 | 只加载变更影响域 | 只加载直接依赖 |

### 4.3 变更定界（Scope Bounding）

S3/S4 级别下，**Agent 不得尝试理解全部代码**。必须通过定界机制限定上下文：

1. **直接变更区**：本次修改的文件（Agent 完全理解）
2. **直接依赖区**：import/require 这些文件的模块（Agent 读取接口契约，不读实现）
3. **间接影响区**：间接依赖方（Agent 只读取 API 签名和测试契约）
4. **盲区**：无依赖关系的代码（Agent 不加载）

定界规则：
- S1/S2：可加载直接变更区 + 直接依赖区
- S3：只加载直接变更区，依赖区通过 Contract 文件读取接口
- S4：只加载直接变更区，其他区通过 API Schema + CI 验证

### 4.4 结构化契约（Contract File）

S3/S4 级别下，Contract 文件替代 markdown Spec 作为 P7 的执行载体。Contract 文件定义：

```yaml
contract:
  id: F042
  module: payment-service
  change_type: api_addition
  api: POST /v2/refund
  ...
  test_scope: [payment/refund_test.go, order/refund_integration_test.go]
  nfr:
    latency_p99: 200ms
    error_budget: 0.01%
```

Contract 文件位于 `.contracts/{module}/F{NNN}.yaml`，由状态机在 Phase 2→3 转换时生成。

### 4.5 不可妥协项（任何 Scale 必须遵守）
...
## 第 11 章：Lessons Learned Protocol
